...
export default function EstimationTool() {
  ...
  const getOperationSymbol = (op: string): string => {
    const symbols: any = { add: '+', subtract: '−', multiply: '×', divide: '÷' };
    return symbols[op] || op;
  };

  // ===== QUESTION GENERATION =====
  const generateLevel1 = (forcedOperation: string | null = null): any => {
  ...
  const generateLevel2 = (forcedOperation: string | null = null): any => {
    const operations = forcedOperation ? [forcedOperation] : ['add', 'subtract', 'multiply', 'divide'];
    let attempts = 0;
  ...
  const generateLevel3 = (): any => {
    ...
    while (attempts < 50) {
      ...
      let doOp1First = useParentheses || op1IsHighPrecedence;
  ...
  const generateQuestion = (level: string, forcedOperation: string | null = null): any => {
      ...
      <div className="min-h-screen p-8" style={{ backgroundColor: '#f5f3f0' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-center gap-4 mb-8">
            {['whiteboard', 'worked', 'worksheet'].map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={
            ...
            <>
              ...
              {worksheet.length > 0 && (
                <div className="rounded-xl shadow-2xl p-8 relative" style={{ backgroundColor: getQuestionBg() }}>
                  ...
                  {Array.isArray(worksheet[0]) || worksheet[0]?.level ? (
                    <div className="grid grid-cols-3 gap-6">
                      {worksheet.map((levelData: any, idx: number) => {
                        const levelNames: any = { level1: 'Level 1', level2: 'Level 2', level3: 'Level 3' };
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="space-y-3">
                              {levelData.questions.map((q: any, i: number) => (
