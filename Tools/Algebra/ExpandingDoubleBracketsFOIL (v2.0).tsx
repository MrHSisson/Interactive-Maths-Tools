...
export default function ExpandingDoubleBracketsGenerator() {
  ...
  const FoilDisplay = ({ q }) => {
    ...
    const b1t1Ref = useRef(null); // Bracket 1 Term 1
    const b1t2Ref = useRef(null); // Bracket 1 Term 2
    const b2t1Ref = useRef(null); // Bracket 2 Term 1
    const b2t2Ref = useRef(null); // Bracket 2 Term 2
    ...
    const formatFirstTerm = (coeff) => {
      if (coeff === 1) return 'x';
      if (coeff === -1) return '−x';
      return (coeff < 0 ? '−' : '') + Math.abs(coeff) + 'x';
    };

    const formatSecondTerm = (value) => {
    ...
    // Format results
    const formatResult = (value, withVar = false) => {
