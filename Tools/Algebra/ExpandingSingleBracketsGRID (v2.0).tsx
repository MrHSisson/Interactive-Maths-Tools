...
export default function ExpandingSingleBracketsGridMethod() {
  ...
  const TableDisplay = ({ q }) => {
    const v = q.varName || 'x';
    
    if (q.withVariable) {
      const firstHeader = q.b === 1 ? v : q.b === -1 ? '−' + v : q.b + v;
      const secondHeader = q.c >= 0 ? '+' + q.c : q.c;
      
      let firstCell, secondCell, outsideCoeff;
      ...
      const firstHeader = q.isReversed ? q.c : (q.b === 1 ? v : q.b === -1 ? '−' + v : q.b + v);
      const secondHeader = q.isReversed ? '−' + (q.b === 1 ? '' : q.b) + v : (q.c >= 0 ? '+' + q.c : q.c);
      const firstCell = q.isReversed ? q.constant : (q.xCoeff === 1 ? v : q.xCoeff === -1 ? '−' + v : q.xCoeff + v);
      const secondCell = q.isReversed ? (q.xCoeff === 1 ? v : q.xCoeff === -1 ? '−' + v : q.xCoeff + v) : q.constant;
