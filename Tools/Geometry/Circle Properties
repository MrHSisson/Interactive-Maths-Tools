...
export default function CirclePropertiesTool() {
  ...
  const renderSector = (q, size = 200) => {
    const centerX = size / 2;
    const centerY = size / 2;
    const circleRadius = size * 0.35;
    
    // For semi-circle, randomly choose top or bottom half
    let startAngle, endAngle, isTopHalf;
    if (q.theta === 180) {
      isTopHalf = Math.random() > 0.5;
      if (isTopHalf) {
        startAngle = 180; // Start from left
        endAngle = 0; // End at right (going through top)
      } else {
        startAngle = 0; // Start from right
        endAngle = 180; // End at left (going through bottom)
      }
    } else if (q.theta === 90) {
      startAngle = -90;
      endAngle = 0;
    ...
    // Calculate position for dimension arrow and label
    let dimensionLine, labelX, labelY, labelText, labelAngle = 0;
    
    if (q.level === 1) {
      // Level 1: Show diameter across the base (straight edge of semicircle)
      const midX = (startX + endX) / 2;
      const midY = (startY + endY) / 2;
      
      // Calculate angle of the diameter for label rotation
      ...
      // Level 2: Quarter circle - show radius on vertical or horizontal edge
      // Use one of the straight edges
      ...
      // Level 3: Show radius on one of the radii
      const midX = (centerX + startX) / 2;
      const midY = (centerY + startY) / 2;
    ...
    return (
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {/* Manual arrow heads - start point */}
        {(() => {
          ...
          return (
            <>
              ...
              <line
                x1={dimensionLine.x1}
                y1={dimensionLine.y1}
                x2={arrow1X + perpX * arrowWidth}
                y2={arrow1Y + perpY * arrowWidth}
                stroke="#000000"
                strokeWidth="2"
              />
              <line
              ...
              <line
                x1={dimensionLine.x2}
                y1={dimensionLine.y2}
                x2={arrow2X + perpX * arrowWidth}
                y2={arrow2Y + perpY * arrowWidth}
                stroke="#000000"
                strokeWidth="2"
              />
              <line
        ...
        {/* Angle arc and label for Level 3 */}
        {q.level === 3 && (
          <>
            {/* Small arc showing the angle at the center, starting from first radius */}
            <path
              d={`M ${centerX + Math.cos(startRad) * 25} ${centerY + Math.sin(startRad) * 25} A 25 25 0 ${q.theta > 180 ? 1 : 0} 1 ${centerX + Math.cos(endRad) * 25} ${centerY + Math.sin(endRad) * 25}`}
              fill="none"
              stroke="#2E7D32"
              strokeWidth="2"
            />
        ...
        {/* Show theta value only for Level 2 and 3 */}
        {q.level !== 1 && (
          <text
            x={centerX}
            y={centerY + circleRadius + 45}
            fill="#2E7D32"
            fontSize="18"
            fontWeight="bold"
            textAnchor="middle"
  ...
  const generateCircumferenceQuestion = (level, angle) => {
    if (level === 'level1') {
      ...
      const working = findWhat === 'radius' ? [
        ...
        { 
          type: 'substitution', 
          textPi: `r = ${formatNumber(circumferencePi)}π ÷ (2π)`,
          textNumeric: `r = ${formatNumber(Math.round(circumference * 10) / 10)} ÷ (2π)`
        },
        { 
          type: 'calculation', 
          textPi: `r = ${formatNumber(radius)} cm`,
        ...
        { 
          type: 'substitution', 
          textPi: `d = ${formatNumber(circumferencePi)}π ÷ π`,
          textNumeric: `d = ${formatNumber(Math.round(circumference * 10) / 10)} ÷ π`
        },
        { 
          type: 'calculation', 
          textPi: `d = ${formatNumber(diameter)} cm`,
  ...
  const generateAreaQuestion = (level, angle) => {
    if (level === 'level1') {
      const radius = allowDecimals 
        ? Math.round((Math.random() * 24 + 1) * 10) / 10 
        : Math.floor(Math.random() * 25) + 1;
  ...
  const generateSectorQuestion = (level, angle) => {
    let theta, radius, diameter;
    let questionStyle = sectorQuestionStyle;
    
    // If mixed mode, randomly select a question style
    ...
    if (level === 'level1') {
      ...
      let displayQuestion, answer, answerNumeric, answerPi, working;
      ...
      if (questionStyle === 'area') {
        ...
        working = [
          ...
          { 
            type: 'final', 
            answerPi: `${answerPi} cm²`,
            answerNumeric: `${answerNumeric} cm²`
          }
        ];
        ...
        working = [
          ...
          { 
            type: 'calculation', 
            textPi: `Arc length = ${formatNumber(arcLengthPi)}π cm`,
      ...
      // Random angle between 1 and 359
  ...
  const handleGenerateWorksheet = () => {
    ...
    const generateUniqueQuestion = (lvl) => {
      ...
      do {
        ...
        // Create a unique key based on the question type and parameters
        if (questionType === 'sectors') {
          // For sectors: use theta, diameter, and question style
          uniqueKey = `${q.theta}-${q.diameter}-${q.questionStyle}`;
        } else if (questionType === 'circumference') {
          // For circumference: use given measurement, level, and find type
          if (q.given === 'radius') {
          ...
          // For area: use given measurement, level, and find type
          if (q.given === 'radius') {
        ...
        attempts++;
        if (attempts > 100) break; // Safety break to avoid infinite loop
    ...
    if (isDifferentiated) {
      ['level1', 'level2', 'level3'].forEach(lvl => {
        for (let i = 0; i < numQuestions; i++) {
          const q = generateUniqueQuestion(lvl);
          questions.push({ ...q, difficulty: lvl });
        }
      });
    } else {
      for (let i = 0; i < numQuestions; i++) {
        const q = generateUniqueQuestion(difficulty);
  ...
  const renderCircle = (q, size = 200) => {
    if (q.type === 'sector') {
      return renderSector(q, size);
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex justify-center gap-3 flex-wrap">
            <button
              onClick={() => setQuestionType('circumference')}
              className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
                (questionType === 'circumference'
                  ? 'bg-indigo-600 text-white shadow-lg'
                  : 'bg-white text-indigo-600 border-2 border-indigo-300 shadow')}>
              Circumference
            </button>
            <button
              onClick={() => setQuestionType('area')}
              className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
                (questionType === 'area'
                  ? 'bg-indigo-600 text-white shadow-lg'
                  : 'bg-white text-indigo-600 border-2 border-indigo-300 shadow')}>
            ...
            <button
              onClick={() => setQuestionType('sectors')}
              className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={'px-4 py-2 rounded-lg font-bold text-sm w-24 ' + 
                        (difficulty === 'level1' ? 'bg-green-600 text-white' : 
                    ...
                    <button 
                      ...
                      className={'px-4 py-2 rounded-lg font-bold text-sm w-24 ' + 
                        (difficulty === 'level2' ? 'bg-yellow-600 text-white' : 
                    ...
                    <button 
                      ...
                      className={'px-4 py-2 rounded-lg font-bold text-sm w-24 ' + 
                        (difficulty === 'level3' ? 'bg-red-600 text-white' : 
            ...
            {whiteboardQuestion && (
              <div className="bg-white rounded-xl shadow-2xl p-8">
                  ...
                  {!showWhiteboardAnswer ? (
                    ...
                    <span className="text-emerald-600">
                      {whiteboardQuestion.type === 'circumference' 
                        ? (whiteboardQuestion.level === 3 
                            ? `${whiteboardQuestion.answer} cm` 
                            : `${whiteboardQuestion.circumference} cm`)
                        : (whiteboardQuestion.level === 3 
                            ? `${whiteboardQuestion.answer} cm` 
                            : `${whiteboardQuestion.area} cm²`)}
                    </span>
                  )}
                </div>
                ...
                <div className="flex gap-6">
                  {/* Circle diagram box */}
                  <div className="bg-white rounded-xl flex items-center justify-center" style={{width: '450px', height: '500px'}}>
                    <div>
                      {renderCircle(whiteboardQuestion, 400)}
                    </div>
                  </div>
                  
                  {/* Annotation box */}
                  <div className="flex-1 bg-gray-100 rounded-xl p-6" style={{minHeight: '500px'}}>
                  </div>
                </div>
              </div>
            )}
                ...
                {questionType === 'sectors' && (
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      value={sectorQuestionStyle}
                      onChange={(e) => setSectorQuestionStyle(e.target.value)}
                      className="px-3 py-2 border-2 border-purple-300 rounded-lg text-base font-semibold"
                    >
                      <option value="mixed">Mixed</option>
                      <option value="area">Area</option>
                      <option value="arcLength">Arc Length</option>
                  ...
                  <div className="mt-8 space-y-6">
                    {question.working.map((step, idx) => (
                      <div key={idx} className="bg-gray-50 rounded-lg p-6">
                        ...
                        {step.type === 'formula' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Formula:</h4>
                            <div className="text-3xl font-medium text-blue-600">{step.text}</div>
                          </div>
                        )}
                        ...
                        {step.type === 'rearrange' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Rearrange:</h4>
                            <div className="text-3xl font-medium text-blue-600">{step.text}</div>
                          </div>
                        )}
                        ...
                        {step.type === 'substitution' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Substitute:</h4>
                            <div className="text-3xl font-medium text-blue-600">
                              {answerInPi ? step.textPi : step.textNumeric || step.text}
                            </div>
                          </div>
                        )}
                        ...
                        {step.type === 'calculation' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Calculate:</h4>
                            <div className="text-3xl font-medium text-blue-600">
                              {answerInPi ? step.textPi : step.textNumeric || step.text}
                            </div>
                          </div>
                        ...
                        {step.type === 'findRadius' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Find Radius:</h4>
                            <div className="text-3xl font-medium text-blue-600">{step.text}</div>
                          </div>
                        )}
                        ...
                        {step.type === 'simplify' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Simplify:</h4>
                            <div className="text-3xl font-medium text-blue-600">
                              {answerInPi ? step.textPi : step.textNumeric || step.text}
                            </div>
                          </div>
                        ...
                        {step.type === 'findDiameter' && (
                          <div>
                            <h4 className="text-xl font-semibold text-gray-700 mb-3">Find Diameter:</h4>
                            <div className="text-3xl font-medium text-blue-600">{step.text}</div>
                          </div>
                        )}
                        ...
                        {step.type === 'final' && (
                          <div>
                            ...
                            <div className="text-5xl font-bold text-emerald-600 bg-emerald-50 p-6 rounded-lg text-center">
                              {answerInPi ? step.answerPi : step.answerNumeric || step.answer}
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    {worksheet.map((q, i) => (
                      <div key={i} className="bg-gray-50 rounded-lg p-4 border-2 border-gray-200">
                        <div className="font-bold text-xl text-indigo-900 mb-3 text-center">
                          {i + 1}. {q.displayQuestion}
                        </div>
