...
export default function CirclePropertiesTool() {
  ...
  const renderSector = (q: any, size: number = 200) => {
    const centerX = size / 2;
    const centerY = size / 2;
    const circleRadius = size * 0.35;
    
    let startAngle: number, endAngle: number, isTopHalf: boolean;
    if (q.theta === 180) {
    ...
    let dimensionLine: any, labelX: number, labelY: number, labelText: string, labelAngle = 0;
    ...
    return (
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <line x1={dimensionLine.x1} y1={dimensionLine.y1} x2={dimensionLine.x2} y2={dimensionLine.y2}
        ...
        {(() => {
          ...
          return (
            <>
              <line x1={dimensionLine.x1} y1={dimensionLine.y1}
                x2={arrow1X + perpX * arrowWidth} y2={arrow1Y + perpY * arrowWidth}
                stroke="#000000" strokeWidth="2" />
              <line x1={dimensionLine.x1} y1={dimensionLine.y1}
                x2={arrow1X - perpX * arrowWidth} y2={arrow1Y - perpY * arrowWidth}
                stroke="#000000" strokeWidth="2" />
              <line x1={dimensionLine.x2} y1={dimensionLine.y2}
        ...
        <g transform={`translate(${labelX}, ${labelY}) rotate(${q.level === 1 ? 0 : labelAngle})`}>
          ...
          <text x="0" y="0" fill="#000000" fontSize="18" fontWeight="bold"
            textAnchor="middle" dominantBaseline="middle">{labelText}</text>
        </g>
        
        {q.level === 3 && (
          <>
            <path d={`M ${centerX + Math.cos(startRad) * 25} ${centerY + Math.sin(startRad) * 25} A 25 25 0 ${q.theta > 180 ? 1 : 0} 1 ${centerX + Math.cos(endRad) * 25} ${centerY + Math.sin(endRad) * 25}`}
            ...
            {(() => {
              ...
              return (
                <text x={thetaLabelX} y={thetaLabelY} fill="#2E7D32" fontSize="18"
        ...
        {q.level !== 1 && (
          <text x={centerX} y={centerY + circleRadius + 45} fill="#2E7D32"
            fontSize="18" fontWeight="bold" textAnchor="middle">θ = {q.theta}°</text>
        )}
      </svg>
    );
  };
  ...
  const generateSectorQuestion = (level: string, angle: number) => {
    let theta: number, radius: number, diameter: number;
    ...
    if (level === 'level1') {
      ...
      let displayQuestion: string, answer: string, answerNumeric: string, answerPi: string, working: any[];
  ...
  const renderCircle = (q: any, size: number = 200) => {
    if (q.type === 'sector') {
    ...
    return (
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {(q.given === 'diameter' || q.find === 'diameter') && (
          <>
            <line x1={diamStartX} y1={diamStartY} x2={diamEndX} y2={diamEndY}
              stroke="#DC2626" strokeWidth="2" strokeDasharray="5,5" />
            <rect x={diamLabelX - 28} y={diamLabelY - 12} width="56" height="24"
              fill="white" opacity="0.9" rx="3" />
            <text x={diamLabelX} y={diamLabelY} fill="#DC2626" fontSize="16"
        ...
        {(q.given === 'radius' || q.find === 'radius') && (
          <>
            <line x1={centerX} y1={centerY} x2={radEndX} y2={radEndY}
              stroke="#059669" strokeWidth="2" strokeDasharray="5,5" />
            <rect x={radLabelX - 26} y={radLabelY - 12} width="52" height="24"
              fill="white" opacity="0.9" rx="3" />
            <text x={radLabelX} y={radLabelY} fill="#059669" fontSize="16"
              fontWeight="bold" textAnchor="middle" dominantBaseline="middle">
        ...
        {q.given === 'circumference' && (
          <text x={centerX} y={centerY + circleRadius + 45} fill="#7C3AED"
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-50 to-blue-100 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-center gap-3 flex-wrap mb-6">
          <button onClick={() => setQuestionType('circumference')}
            className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
              (questionType === 'circumference' ? 'bg-purple-600 text-white shadow-lg' : 'bg-white text-purple-600 shadow')}>
            Circumference
          </button>
          <button onClick={() => setQuestionType('area')}
            className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
              (questionType === 'area' ? 'bg-purple-600 text-white shadow-lg' : 'bg-white text-purple-600 shadow')}>
            Area
          </button>
          <button onClick={() => setQuestionType('sectors')}
            className={'px-8 py-4 rounded-xl font-bold text-xl transition-all ' +
              (questionType === 'sectors' ? 'bg-purple-600 text-white shadow-lg' : 'bg-white text-purple-600 shadow')}>
            Sectors
          </button>
        </div>
              ...
              <div className="bg-white rounded-xl shadow-2xl p-8">
                ...
                <div className="flex gap-6">
                  <div className="bg-white rounded-xl flex items-center justify-center" style={{width: '450px', height: '500px'}}>
                    <div>{renderCircle(whiteboardQuestion, 400)}</div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    {['level1', 'level2', 'level3'].map((lvl, idx) => (
                      <div key={lvl} className={'rounded-xl p-6 border-4 ' +
                        ...
                        <div className="space-y-4">
                          {worksheet.filter(q => q.difficulty === lvl).map((q: any, i: number) => (
