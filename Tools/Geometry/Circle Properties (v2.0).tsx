...
export default function CirclePropertiesTool() {
  ...
  const renderCircleDiagram = (q: any, size: number = 400, isWorksheet: boolean = false, fontSizeScale: number = 1, levelOverride?: string): JSX.Element => {
    if (q.type === 'sector') {
  ...
  const renderSectorDiagram = (q: any, size: number = 400, isWorksheet: boolean = false, fontSizeScale: number = 1, levelOverride?: string): JSX.Element => {
    const centerX = size / 2;
    const centerY = size / 2;
    const circleRadius = size * 0.35;
    ...
    let dimensionLineAdjusted: any, labelXAdjusted: number, labelYAdjusted: number;
  ...
  // ===== QUESTION GENERATION =====
  const generateCircumferenceQuestion = (level: string, angle: number): any => {
    if (level === 'level1') {
  ...
  const generateAreaQuestion = (level: string, angle: number): any => {
    if (level === 'level1') {
  ...
  const generateSectorQuestion = (level: string, angle: number): any => {
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: '#f5f3f0' }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-center gap-3 flex-wrap mb-6">
          {['circumference', 'area', 'sectors'].map((topic) => (
